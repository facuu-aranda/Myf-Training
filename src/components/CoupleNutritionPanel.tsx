...
export function CoupleNutritionPanel({ userId, profiles }: { userId: string; profiles: Profile[] }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setSummaries(next)
    } catch { setError(t('couple.nutritionError')) } finally { setIsLoading(false) }
