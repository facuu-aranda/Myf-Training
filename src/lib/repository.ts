...
function exerciseFromRow(row: Row): Exercise {
  ...
  return { id: stringValue(row.id), externalId: stringValue(row.external_id), name: stringValue(row.name), nameEs: stringValue(row.name_es, stringValue(row.name)), description: stringValue(row.description), instructions: arrayValue(instructions.en), instructionsEs: arrayValue(instructions.es), category: stringValue(row.category), muscleGroup: stringValue(row.muscle_group), target: stringValue(row.target), equipment: stringValue(row.equipment), gifUrl: stringValue(row.video_url) || undefined, imageUrl: stringValue(row.image_url) || stringValue(row.thumbnail_url) || undefined, source: stringValue(row.source, 'exercises-dataset'), sourceUrl: stringValue(row.source_url), ownerUserId: typeof row.owner_user_id === 'string' ? row.owner_user_id : null, ownerSpaceId: typeof row.owner_space_id === 'string' ? row.owner_space_id : null, visibility: stringValue(row.visibility, 'system') as Exercise['visibility'], status: stringValue(row.status, 'active') as Exercise['status'] }
}
