...
import { loadRecipes, loadUserCoupleId, searchFoods } from '../lib/repository'
...
function emptyLog(userId: string, date: string, coupleId: string | null): LogDraft { return { id: crypto.randomUUID(), userId, coupleId, visibility: 'private', consumedOn: date, consumedAt: localDateTimeValue(), mealType: 'other', notes: '', items: [] } }
...
export function FoodLogPage() {
  ...
  useEffect(() => { if (user) void Promise.all([loadUserCoupleId(user.id), loadRecipes()]).then(([nextCoupleId, nextRecipes]) => { setCoupleId(nextCoupleId); setRecipes(nextRecipes) }).catch(() => { setCoupleId(null); setRecipes([]) }) }, [user])
...
function FoodLogEditor({ initial, language, recipes, saving, onCancel, onSave, t }: { initial: LogDraft; language: 'en' | 'es'; recipes: Recipe[]; saving: boolean; onCancel: () => void; onSave: (log: FoodLog) => Promise<void>; t: (key: string) => string }) {
  ...
  const submit = (event: FormEvent) => {
    ...
    void onSave({ id: draft.id, userId: draft.userId, coupleId: draft.coupleId, visibility: draft.visibility, consumedOn: draft.consumedOn, consumedAt: consumedAt.toISOString(), mealType: draft.mealType, notes: draft.notes, items, createdAt: '', updatedAt: '' })
  }
