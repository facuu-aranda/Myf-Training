...
import { loadGroceryList, loadGroceryLists, loadMealPlansForHousehold, loadRecipes, loadUserHouseholdId, saveGroceryList } from '../lib/repository'
...
function emptyList(userId: string, householdId: string, startsOn: string, endsOn: string): GroceryList { return { id: uid('grocery-list'), householdId, createdBy: userId, startsOn, endsOn, status: 'current', items: [], createdAt: '', updatedAt: '' } }
...
export function GroceryPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    setError('')
    try {
      const nextHouseholdId = await loadUserHouseholdId(user.id)
      setHouseholdId(nextHouseholdId)
      if (!nextHouseholdId) { setList(null); setHistory([]); return }
      const [current, previous] = await Promise.all([loadGroceryList(nextHouseholdId, startsOn, endsOn), loadGroceryLists(nextHouseholdId)])
      setList(current)
      setHistory(previous)
  ...
  const generate = async () => {
    ...
    setError('')
    try {
      const [plans, recipes] = await Promise.all([loadMealPlansForHousehold(user.id, householdId, startsOn, endsOn), loadRecipes()])
      const calculated = generateGroceryItems(plans, recipes)
      const base = list ?? emptyList(user.id, householdId, startsOn, endsOn)
