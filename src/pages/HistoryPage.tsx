import { CalendarDays, ChevronRight, Clock3, Dumbbell, ListChecks, MessageSquare, Share2, Trophy } from 'lucide-react'
...
function SessionDetail({ session, day, exercises, locale, t }: { session: WorkoutSession; day?: { name: string; nameEs: string }; exercises: Array<{ id: string; name: string; nameEs: string }>; locale: string; t: (key: string) => string }) { const language = locale.startsWith('es') ? 'es' : 'en'; const byExercise = session.sets.reduce<Record<string, typeof session.sets>>((grouped, set) => { grouped[set.exerciseId] = [...(grouped[set.exerciseId] ?? []), set]; return grouped }, {}); return <div className="session-detail"><div className="session-detail-head"><div><span className="eyebrow-label">{formatDate(session.finishedAt ?? session.startedAt, locale)}</span><h3>{day ? localizedName(day, language) : t('history.details')}</h3></div><StatusPill tone="green" dot>{t('history.completed')}</StatusPill></div><div className="session-detail-summary"><span><Clock3 size={14} />{Math.round(session.durationSeconds / 60)} {t('common.min')}</span><span><Dumbbell size={14} />{formatNumber(calculateSessionVolume(session))} {t('common.kg')}</span><span><ListChecks size={14} />{session.sets.length} {t('common.sets')}</span><span><Trophy size={14} />{session.overallFeeling}/5</span></div><div className="session-exercise-list">{Object.entries(byExercise).map(([exerciseId, sets]) => { const exercise = exercises.find((item) => item.id === exerciseId); return <div className="session-exercise" key={exerciseId}><div className="session-exercise-title"><strong>{exercise ? localizedName(exercise, language) : 'Exercise'}</strong><span>{sets.length} {t('common.sets')}</span></div>{sets.map((set) => <div className="session-set" key={set.id}><span>{t('live.set')} {set.setNumber}</span><strong>{set.actualWeight} {t('common.kg')} × {set.actualReps}</strong><span>RPE {set.difficulty}</span>{set.notes && <MessageSquare size={12} />}</div>)}</div> })}</div>{session.notes && <p className="session-note">{session.notes}</p>}</div> }
