...
export function LiveTrainingPage() {
  ...
  const begin = () => {
    ...
    setSessionId(id); setExerciseIndex(0); setSetIndex(0); setDraft(blankDraft(selectedDay.exercises[0])); setPhase('set')
  }
  ...
  const moveNext = useCallback((additionalSets: ExerciseSet[] = []) => {
    ...
    advanceToPosition(getNextLivePosition(selectedDay, completed))
  }, [advanceToPosition, liveSession, selectedDay])
  ...
  const completeSet = () => {
    ...
    if (completedForPlan >= currentPlan.sets) { moveNext(); return }
    ...
    if (currentPlan.restSeconds > 0 && (completedForPlan + 1 < currentPlan.sets || exerciseIndex + 1 < (selectedDay?.exercises.length ?? 0))) {
      const nextPosition = selectedDay ? getNextLivePosition(selectedDay, [...(liveSession?.sets ?? []), ...(recordedSet ? [recordedSet] : [])]) : null
  ...
  const finish = () => {
    ...
    setPhase('ready'); setSessionId(''); navigate('/app', { replace: true })
  }

  const discard = () => { if (activeSession) abandonSession(activeSession.id); setSessionId(''); setPhase('ready') }
  if (!user) return null
