...
import { deleteRecipe, loadRecipes, loadUserHouseholdId, saveRecipe, searchFoods } from '../lib/repository'
...
function emptyRecipe(userId: string, householdId: string | null): RecipeDraft {
  return { id: uid('recipe'), createdBy: userId, householdId, name: '', nameEs: '', description: '', instructions: '', prepTimeMinutes: 0, cookTimeMinutes: 0, servings: 1, imageUrl: '', visibility: 'private', ingredients: [], createdAt: '', updatedAt: '' }
}

function draftFromRecipe(recipe: Recipe): RecipeDraft {
  return { ...recipe, householdId: recipe.householdId ?? null, createdBy: recipe.createdBy ?? '', ingredients: recipe.ingredients.map((ingredient) => ({ id: ingredient.id, food: ingredient.food ?? null, quantity: String(ingredient.quantity), unit: ingredient.unit, portion: ingredient.portion ?? null, notes: ingredient.notes })) }
}
...
export function RecipesPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    setError('')
    try {
      const [nextRecipes, nexthouseholdId] = await Promise.all([loadRecipes(), loadUserHouseholdId(user.id)])
  ...
  if (!user) return null
  return <PageMotion><div className="page-header"><div><span className="eyebrow-label">{t('nav.nutrition')}</span><h1>{t('nutrition.recipes')}</h1><p>{t('nutrition.recipesSubtitle')}</p></div><NeonButton size="sm" onClick={() => setEditor(emptyRecipe(user.id, householdId))}><Plus size={14} />{t('nutrition.createRecipe')}</NeonButton></div><NutritionSubnav />{error && <div className="inline-error" role="alert">{error}</div>}{isLoading ? <LoadingState /> : recipes.length ? <section className="recipe-library"><SectionHeading eyebrow={t('nutrition.recipes')} title={`${recipes.length} ${t('nutrition.recipeCount')}`} /><div className="recipe-grid">{recipes.map((recipe) => <RecipeCard key={recipe.id} recipe={recipe} language={language} onEdit={() => setEditor(draftFromRecipe(recipe))} onDelete={() => { void remove(recipe) }} t={t} />)}</div></section> : <GlassCard><EmptyState icon={<ChefHat size={20} />} title={t('nutrition.emptyRecipes')} description={t('nutrition.recipesSubtitle')} action={<NeonButton size="sm" onClick={() => setEditor(emptyRecipe(user.id, householdId))}><Plus size={14} />{t('nutrition.createRecipe')}</NeonButton>} /></GlassCard>}<Modal open={Boolean(editor)} onClose={() => { if (!saving) setEditor(null) }} title={editor?.name ? t('nutrition.editRecipe') : t('nutrition.createRecipe')} size="lg">{editor && <RecipeEditor initial={editor} householdId={householdId} language={language} saving={saving} onCancel={() => setEditor(null)} onSave={save} t={t} />}</Modal></PageMotion>
}
...
function RecipeEditor({ initial, householdId, language, saving, onCancel, onSave, t }: { initial: RecipeDraft; householdId: string | null; language: 'en' | 'es'; saving: boolean; onCancel: () => void; onSave: (recipe: Recipe) => Promise<void>; t: (key: string) => string }) {
