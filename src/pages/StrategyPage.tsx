...
import { getStrategyManagement } from '../lib/coaching-space'
import { saveStrategyVersion } from '../lib/strategy-version'
import type { Exercise, NutritionPlan, StrategyManagement, WorkoutDay, WorkoutExercise } from '../types'
...
export function StrategyPage() {
  ...
  const [strategyManagement, setStrategyManagement] = useState<StrategyManagement | null>(null)
  ...
  useEffect(() => {
    if (!user) return
    let active = true
    void getStrategyManagement(user.id).then((management) => { if (active) setStrategyManagement(management) }).finally(() => { if (active) setManagementLoaded(true) })
    return () => { active = false }
  }, [user])

  if (!user || !nutrition || !profile) return null
  const language = i18n.language.startsWith('es') ? 'es' : 'en'
  const strategyLocked = managementLoaded && strategyManagement?.managementMode === 'coach' && strategyManagement.managerUserId !== user.id
  ...
  const moveExercise = (planId: string, offset: number) => {
    if (strategyLocked || !selectedDay) return
    const ids = selectedDay.exercises.slice().sort((left, right) => left.orderIndex - right.orderIndex).map((plan) => plan.id)
  ...
  const saveVersion = async () => {
    if (strategyLocked) return
    try {
      await saveStrategyVersion(user.id, { nutrition, workoutDays: userDays })
      setVersionNotice(t('strategy.versionSaved'))
    } catch {
      setVersionNotice(t('strategy.versionError'))
    }
    window.setTimeout(() => setVersionNotice(''), 3000)
  }

  return <PageMotion>
    <div className="page-header"><div><span className="eyebrow-label">{t('nav.strategy')}</span><h1>{t('strategy.title')}</h1><p>{t('strategy.subtitle')}</p></div><div style={{ display: 'flex', gap: 8, alignItems: 'center' }}><FeatureGate entitlement="strategy_versions_personal" plan="Plus"><NeonButton variant="secondary" size="sm" onClick={() => { void saveVersion() }} disabled={strategyLocked}>{t('strategy.saveVersion')}</NeonButton></FeatureGate><NeonButton variant="secondary" size="sm" onClick={() => addWorkoutDay(user.id)} disabled={strategyLocked}><Plus size={14} />{t('strategy.addDay')}</NeonButton></div></div>{versionNotice && <div className="inline-success" role="status">{versionNotice}</div>}{strategyLocked && <div className="coach-managed-banner" role="status"><span>{t('strategy.managedByCoach')}</span><small>{t('strategy.managedByCoachHint')}</small></div>}
    <div className="strategy-top"><GlassCard className="nutrition-card"><div className="nutrition-head"><div><span className="eyebrow-label">{t('strategy.nutrition')}</span><h2>{nutrition.calories} kcal / day</h2><p>{nutrition.notes}</p></div><IconButton label={t('strategy.editNutrition')} onClick={() => setNutritionOpen(true)} disabled={strategyLocked}><Pencil size={15} /></IconButton></div><div className="nutrition-grid"><NutritionStat label={t('strategy.calories')} value={nutrition.calories} unit="kcal" /><NutritionStat label={t('strategy.protein')} value={nutrition.protein} unit="g" /><NutritionStat label={t('strategy.carbs')} value={nutrition.carbs} unit="g" /><NutritionStat label={t('strategy.fats')} value={nutrition.fats} unit="g" /></div></GlassCard><GlassCard className="activity-targets"><SectionHeading eyebrow={t('strategy.activity')} title={t('strategy.activity')} /><div className="target-list"><TargetRow icon={<Target size={14} />} label={t('strategy.steps')} value={profile.dailyStepGoal} unit={t('common.steps')} progress={profile.dailyStepGoal / 12000 * 100} /><TargetRow icon={<Utensils size={14} />} label={t('strategy.calories')} value={profile.dailyCalorieGoal} unit="kcal" progress={profile.dailyCalorieGoal / 2500 * 100} /><TargetRow icon={<Dumbbell size={14} />} label={t('strategy.workouts')} value={userDays.length} unit={t('common.week')} progress={userDays.length / 7 * 100} /></div></GlassCard></div>
    <section className="training-plan"><SectionHeading eyebrow={t('strategy.schedule')} title={t('strategy.trainingPlan')} description={t('strategy.trainingPlanSubtitle')} action={<StatusPill tone="violet" dot>{userDays.length} {t('common.exercises')}</StatusPill>} /><div className="day-tabs">{userDays.map((day) => <button type="button" draggable={!strategyLocked} onDragStart={() => setDraggedDay(day.id)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => onDayDrop(event, day.id)} className={`day-tab ${day.id === selectedDay?.id ? 'day-tab-active' : ''}`} key={day.id} onClick={() => setSelectedDayId(day.id)}><GripVertical size={12} style={{ float: 'right', color: '#6f647d' }} /><strong>{t(`days.${weekdayKey(day.weekday)}`)}</strong><span>{localizedName(day, language)} · {day.exercises.length} {t('common.exercises')}</span></button>)}</div>{!userDays.length && <GlassCard><EmptyState icon={<Dumbbell size={20} />} title={t('strategy.noDays')} description={t('strategy.noDaysSubtitle')} action={<div className="workout-template-actions"><NeonButton size="sm" onClick={() => addWorkoutDay(user.id)} disabled={strategyLocked}><Plus size={14} />{t('strategy.addDay')}</NeonButton><span>{t('strategy.templates')}</span>{([3, 4, 5] as const).map((dayCount) => <NeonButton key={dayCount} size="sm" variant="secondary" onClick={() => createWorkoutTemplate(user.id, dayCount)} disabled={strategyLocked}>{t(`strategy.template${dayCount}`)}</NeonButton>)}</div>} /></GlassCard>}{selectedDay && <GlassCard className="plan-card"><div className="plan-header"><div><div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><h2>{localizedName(selectedDay, language)}</h2><IconButton label={t('strategy.renameDay')} onClick={() => setDayEditor(selectedDay)} disabled={strategyLocked}><Pencil size={13} /></IconButton></div><p>{selectedDay.description} · {selectedDay.estimatedMinutes} {t('common.min')}</p></div><div style={{ display: 'flex', gap: 8 }}><NeonButton variant="secondary" size="sm" onClick={() => setDayEditor(selectedDay)} disabled={strategyLocked}><Pencil size={13} />{t('common.edit')}</NeonButton><IconButton label={t('strategy.duplicateDay')} onClick={() => duplicateWorkoutDay(user.id, selectedDay.id)}><Copy size={14} /></IconButton><NeonButton size="sm" onClick={() => setPickerOpen(true)}><Plus size={14} />{t('strategy.addExercise')}</NeonButton></div></div><div className="plan-list">{selectedDay.exercises.length ? selectedDay.exercises.slice().sort((a, b) => a.orderIndex - b.orderIndex).map((plan, index) => { const exercise = exercises.find((item) => item.id === plan.exerciseId); return <PlanRow readOnly={strategyLocked} key={plan.id} plan={plan} exercise={exercise} index={index} language={language} onOpen={() => exercise && setSelectedExercise(exercise)} onChange={(patch) => { if (!strategyLocked) updateWorkoutExercise(plan.id, patch) }} onRemove={() => removeExerciseFromDay(plan.id)} onDuplicate={() => { if (!strategyLocked) duplicateWorkoutExercise(selectedDay.id, plan.id) }} onMove={(offset) => moveExercise(plan.id, offset)} onDragStart={() => setDraggedExercise(plan.id)} onDragOver={(event) => event.preventDefault()} onDrop={onExerciseDrop} t={t} /> }) : <EmptyState icon={<Dumbbell size={19} />} title={t('strategy.noExercises')} description={t('strategy.emptyDay')} action={<NeonButton size="sm" onClick={() => setPickerOpen(true)}><Plus size={13} />{t('strategy.addExercise')}</NeonButton>} />}</div></GlassCard>}</section>
    <Modal open={nutritionOpen} onClose={() => setNutritionOpen(false)} title={t('strategy.editNutrition')}><NutritionForm nutrition={nutrition} onSave={(patch) => { if (strategyLocked) return; updateNutrition(user.id, patch); if (patch.calories) updateProfile(user.id, { dailyCalorieGoal: patch.calories }); setNutritionOpen(false) }} t={t} /></Modal>
    ...
    <Modal open={Boolean(dayEditor)} onClose={() => setDayEditor(null)} title={t('strategy.renameDay')}><DayForm day={dayEditor} onSave={(patch) => { if (strategyLocked) return; if (dayEditor) updateWorkoutDay(dayEditor.id, patch); setDayEditor(null) }} onDelete={() => { if (strategyLocked) return; if (dayEditor) removeWorkoutDay(dayEditor.id); setDayEditor(null); setSelectedDayId('') }} t={t} /></Modal>
