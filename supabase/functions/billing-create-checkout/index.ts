...
Deno.serve(async (request) => {
  ...
  if (!accessToken || environment !== 'test' || !appUrl) return response({ error: 'billing_provider_unavailable' }, 503)
  ...
  const providerResponse = await fetch('https://api.mercadopago.com/preapproval', { method: 'POST', headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ reason: `Nuvia ${String((plan as Record<string, unknown> | null)?.name ?? 'subscription')}`, external_reference: String(intentData.id), payer_email: auth.data.user.email, auto_recurring: { frequency: 1, frequency_type: interval, transaction_amount: amount, currency_id: price.data.currency }, back_url: `${appUrl.replace(/\/$/, '')}/app/billing/return`, status: 'pending' }) })
  if (!providerResponse.ok) return response({ error: 'provider_checkout_failed' }, 502)
